...
def _queue_invoice_review_request(invoice_id, invoice_number, customer_name, total_amount, invoice_status):
    ...
    notify_module_approver(
        ...
        ref_id=invoice_id,
        subject=f"[PORTMAN] FINV01 Invoice {invoice_number} - Review Required",
        fallback_module='FIN01',
        body_html=f"""<p>Hello Approver,</p>
<p>A FINV01 invoice entry was generated by <strong>{session.get('username')}</strong>.</p>
<p><strong>Invoice No:</strong> {invoice_number}<br>
<strong>Customer:</strong> {customer_name or ''}<br>
<strong>Total Amount:</strong> {float(total_amount or 0):.2f}<br>
<strong>Current Status:</strong> {invoice_status or 'Generated'}</p>
<p>Please review/approve it as per your process in PORTMAN.</p>
<p><a href="{invoice_url}">Open invoice in PORTMAN</a></p>
